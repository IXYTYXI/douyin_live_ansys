...
class CapacityTest(unittest.TestCase):
    def setUp(self):
        ...
        self.p=Pipeline(Path(self.tmp.name),max_inflight=1)
        with self.p.db() as db:
            db.execute("INSERT INTO sessions VALUES ('s',0)")
            for i in range(2):
                db.execute('INSERT INTO segments(id,recording_id,session_id,start,end,audio) VALUES (?,?,?,?,?,?)',(str(i),'r','s',i,i+1,str(i)+'.wav'))
    ...
    def test_inflight_cap_and_stable_audio_url(self):
        ...
        with self.p.db() as db:
            self.assertEqual(db.execute("SELECT count(*) FROM segments WHERE state='submitted'").fetchone()[0],1)
    def test_throttle_pauses_whole_queue(self):
        ...
        with self.p.db() as db:
            self.assertEqual(db.execute("SELECT attempts FROM segments WHERE id='0'").fetchone()[0],1)
    ...
    def test_namespace_cannot_change_on_existing_database(self):
        with self.assertRaises(ValueError): Pipeline(Path(self.tmp.name),business='taobao')
