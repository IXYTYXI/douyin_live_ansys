...
组合联调：给 asr_preview 增加 `--metrics-session ID`，可读取一个 live_room_id 以 synthetic- 开头的模拟场次人数，将相对时间起点与真实音频样本对齐。仅用于本地测试，原数据库时间与记录不改写。前端显示规则生成的主题、关键词和运营测试稿，并标注模拟指标/真实转写的不同来源；无转写时段不复制文字。真实音频仅在覆盖区间可播放。
