export function remoteSession(data){
 ...
 onlineSamples:data.samples.map(r=>({t:r.t,value:r.value})),transcriptLines:asr?(data.segments||[]).filter(r=>r.state==='done').map(r=>({t:r.start,text:r.text})):[],processing:asr?(data.segments||[]).some(r=>r.state!=='done'):false};
}
