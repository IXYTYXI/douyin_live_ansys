...
export function samples(session,start,end){if(['anchor-review','backend-test'].includes(session.source))return (session.onlineSamples||[]).filter(r=>r.t>=start&&r.t<end);let list=[];for(let t=start;t<end;t+=10){const missing=session.id==='math'&&t>=2520&&t<2580;const v=Math.round(90+t/60*1.05+35*Math.sin(t/350)+12*Math.sin(t/80)+(session.id==='reading'?35:0));list.push({t,value:missing?null:Math.max(20,v)});}return list;}
...
export function transcripts(session,start,end){if(['anchor-review','backend-test'].includes(session.source))return (session.transcriptLines||[]).filter(r=>r.t>=start&&r.t<end);const list=[];for(let t=Math.floor(start/120)*120;t<end;t+=120){if(t<start)continue;const ix=Math.floor(t/600)%topics.length;const prompts=session.id==='math'?['先找一找，这道题中谁是单位“1”？','大家试着画一张线段图，把已知条件标出来。','现在把你的答案打在评论区，说一说为什么。','注意这里的对应关系，我们换一种方法检查。','这一步很关键，先理解，再列式计算。']:['先读这一段，找出作者想表达的核心意思。','圈出关键词，再用自己的话概括。','请在评论区写出你的判断和依据。','我们看看这个答案，哪些信息还可以补充？','回到原文，用句子支持你的观点。'];list.push({t,text:prompts[Math.floor(t/120)%5],topic:session.id==='math'?topics[ix]:['阅读导入','关键词定位','主旨归纳','互动答疑'][ix%4]});}return list;}
export function summary(session,a,b){if(['anchor-review','backend-test'].includes(session.source))return {theme:'',keywords:[],body:''};const rows=samples(session,a,b),st=stats(rows),text=transcripts(session,a,b),names=[...new Set(text.map(t=>t.topic))];return {theme:periodTheme(session,text),keywords:session.id==='math'?'单位“1”、线段图、对应关系':'关键词、中心句、原文依据',body:`本时段围绕${names.join('、')}展开，通过讲解和提问推进练习。有效采样点均值 ${st.average??'未获取'} 人，采样最大值 ${st.max??'未获取'} 人。${st.missing?'存在采样缺失，需结合录像核对。':'可结合互动片段进一步复盘。'}人数变化仅作观察，不据此判断教学效果。`};}
